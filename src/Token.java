public class Token {
    ...
    public enum TokenTypes {
        ...
        T_GLOBAL, // variable scope (global)
        T_GOTO, // goto keyword (goto)
        T_HALT_COMPILER, // halts the compiler execution (__halt_compiler)
        T_IF, // if keyword (if)
        T_IMPLEMENTS, // interface implementation (implements)
        T_INC, // incrementing operator (++)
        T_INCLUDE, // include keyword (include())
        T_INCLUDE_ONCE, // includes and evaluates the file during the execution of the script (include_once())
        T_INLINE_HTML, // text outside PHP (HTML text)
        T_INSTANCEOF, // instance of a class (instanceof)
        T_INSTEADOF, // trait (code reuse) conflict resolution keyword (insteadof)
        T_INT_CAST, // casting to integer ( (int) or (integer) )
        T_INTERFACE, // interface keyword (interface)
        T_ISSET, // determine if a variable is set and is not NULL (isset())
        ...
        T_LINE, // line magic constant (__LINE__)
        T_LIST // assign variables as if they were an array (list())
    }
